...
readonly MANIFEST="$WORK/cx-history-ranges.manifest"
...
while IFS='|' read -r label start end extra; do
  [[ -z "${extra:-}" ]]
  [[ "$label" =~ ^[a-z0-9-]+$ ]]
  [[ "$start" =~ ^[0-9]+$ ]]
  [[ "$end" =~ ^[0-9]+$ ]]
  (( end > start ))
  [[ ! -e "$WORK/cx-$label.csv" ]]
  [[ ! -e "$WORK/cx-$label.csv.partial" ]]
  [[ ! -e "$WORK/cx-$label-summary.log" ]]
done <"$MANIFEST"
...
while IFS='|' read -r label start end; do
  runuser -u "${AUDIT_USER:-$(id -un)}" -- \
    ...
    -start "$start" \
    -end "$end" \
    -output "$WORK/cx-$label.csv" \
    -cache-mb 4096 \
    -handles 4096 | tee "$WORK/cx-$label-summary.log"
done <"$MANIFEST"

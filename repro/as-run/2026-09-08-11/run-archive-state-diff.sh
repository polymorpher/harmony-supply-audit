...
readonly BIN="$WORK/harmony-historical-state-diff-amd64"
readonly EXPECTED_BIN_SHA256=${EXPECTED_BIN_SHA256:?set EXPECTED_BIN_SHA256}

: "${DIFF_LABEL:?DIFF_LABEL is required}"
: "${OLD_ROOT:?OLD_ROOT is required}"
: "${NEW_ROOT:?NEW_ROOT is required}"

[[ "$DIFF_LABEL" =~ ^[a-z0-9-]+$ ]]
[[ "$OLD_ROOT" =~ ^0x[0-9a-f]{64}$ ]]
[[ "$NEW_ROOT" =~ ^0x[0-9a-f]{64}$ ]]

readonly OUTPUT="$WORK/$DIFF_LABEL.csv"
readonly LOG="$WORK/$DIFF_LABEL-summary.log"
